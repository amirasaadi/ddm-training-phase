...
for iter in range(nsub):
    ...
    Bound = np.random.uniform(10, 30)
    timelap = np.linspace(0, Time, int(Time/dt))   
    ndt = np.random.uniform(150, 450)
    SP_Var = 0
    drif_Var = 0
    drift_rate = np.random.uniform(.1, .8)
    
    allParam[iter] = [Bound, drift_rate, ndt]
    ...
    dt = 1  # ms
    Time = 1000 * 5 / dt
    B0 = np.random.uniform(10, 30)
    K0 = np.random.uniform(.1, .8)
    SP = 0
    ndt0 = np.random.uniform(150, 450)
    Vars0 = [B0, K0, ndt0]
    ...
    def DDMModel(Vars):
        ...
        #### Main Loop ####
        for t in range(len(timelap) - 1):
            dX = np.random.normal(k_in * AllTr, np.ones([1, len(AllTr)]))
            V[:, t + 1] = V[:, t] + dX * dt
    ...
    # vars = [b,d,ndt,b0,binf,thalf]
    
    res = gp_minimize(DDMModel,  # the function to minimize
                      [(10, 30), (0.1, 0.8), (150, 450)],  # the bounds on each dimension of x
                      x0=Vars0,  # the starting point
                      n_calls=50,  # the number of evaluations of f including at x0
                      n_random_starts=11    ,  # the number of random initial points
                      random_state=777,
                      verbose=1)
...
b_t = pd.DataFrame({'b_t': allParam[:, 0]})
d_t = pd.DataFrame({'d_t': allParam[:, 1]})
ndt_t = pd.DataFrame({'ndt_t': allParam[:, 2]})
